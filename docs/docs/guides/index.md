...
Elixir and Gleam aren't auto-detected yet, so their guides show you a
`Dockerfile.miren` you can drop into your project. Miren builds from that Dockerfile
instead of guessing. This is the same escape hatch available to every language when
you need full control over the build — see [Using Dockerfile.miren](/languages#using-dockerfilemiren).
