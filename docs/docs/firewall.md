...
**Miren Cloud** connectivity is required for authentication (`miren login`) and cluster registration (`miren server install`, `miren server container install`, `miren server register`). If you're running Miren in standalone mode without cloud features, this isn't required.
